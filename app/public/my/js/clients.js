...
$(function () {
            ...
            success: function(data)
            {
                ...
                $('#clients').append(
                    ...
                    <tbody class='text-success'>
                        ...
                        <td> `+ (data.dni == null ? '' : data.dni ) +` </td>
                        <td> `+data.address+` </td>
                        <td> `+data.city+` </td>
                        <td> <a target="_blank" href="https://wa.me/`+data.phone+`"> `+data.phone+` </a> </td>
   ...
   $(document).on('click','.pagination',function(e){
    ...
    $(document).on('click', '#clients_filter',function(e){
        dateExpired()
    })
    ...
    function dateExpired(){
		...
		success: function(data)
		{
		    ...
		    $.each(data, function(index, value){
		        ...
		        let color = num < 25 ? "success" : num > 35 && num < 39 ? "warning" : num >= 39 ? "danger" : ""
                let cutd = value.total_debt_remaining < 1 ? '<span class="right badge badge-warning">Al dia</span>' : '';

                if(value.total_debt_remaining == 0){

                    $('#date_expired'+value.client_id).html(

                        `   <h4>
                                <a href='client_acount/`+value.client_id+`'> ` +cutd+ `  </a>
                            </h4>
                        `
                    )
    ...
    // Sheet Excel import
    // # NUEVO INSERT --
    $(document).on('submit','#form-sheet-excel-import-clients',function(e){
        ...
        $.ajax({
            type: 'POST',
            url: URI_API+'import_sheet_excel_clients',
            data: formData,
            contentType: false,
            cache: false,
            processData:false,
