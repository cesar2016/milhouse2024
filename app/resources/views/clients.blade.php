...
@section('content')
    ...
    <div class="row mb-3">
        <div class="col-3">
            ...
            {{-- <a id="btn-new-product" class="btn btn-app bg-primary" href=" {{ url('#') }} ">
                Descargar Modelo <i class="fas fa-arrow-alt-circle-down"></i>
            </a> --}}

        </div>
        <div class="col-4 mt-3">
            {{-- <form id="form-sheet-excel-import-clients" enctype="multipart/form-data">
                @CSRF
            ...
            <tbody>
                @foreach ($clients as $client)
                    <tr id="{{ "row".$client['id'] }}">
                        ...
                        <td>{{ $client['city'] }}</td>
                        <td><a target="_blank" href="https://wa.me/{{ $client['phone'] }}"> {{ $client['phone'] }}</a></td>
                        <td>{{ $client['email'] ?? ''}}</td>
