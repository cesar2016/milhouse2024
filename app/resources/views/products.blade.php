...
@section('content')
    ...
    <div class="row mb-3">
        <div class="col-3">
            ...
            {{-- <a id="btn-new-product" class="btn btn-app bg-primary" href=" {{ url('dowload_eje_products') }} ">
                Descargar Modelo <i class="fas fa-arrow-alt-circle-down"></i>
            </a> --}}

        </div>
        <div class="col-4 mt-3">
            {{-- <form action="{{ url('products/import_sheet_front') }}" method="POST" enctype="multipart/form-data"> --}}
            {{-- <form id="form-sheet-excel-import" enctype="multipart/form-data">
                @CSRF
                <div class="input-group">
                    <div class="custom-file">
                        <input type="file" id="file" name="file" class="custom-file-input" />
                        <label class="custom-file-label" for="inputGroupFile04">Elija su Excel ...</label>
                    </div>
                    <div class="input-group-append">
                        <button type="submit" class="btn btn-outline-secondary bg-success" type="button" id="inputGroupFileAddon04">Subir
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-2">
                            <input type="date" name="date_purchase" id="date_purchase" class="form-control"
                                placeholder="Fecha Cpra." value="<?php echo date('Y-m-d'); ?>">
                        </div>
                        <div class="col-2">

                            <select name="category_id" id="category_id" class="miSelect1"
                                style='width: 250px; margin-left:5px;' name="state">
                                <option disabled selected value="0" id='default1' style="display:none"> Elija
                                    categoría... </option>
                        ...
                        <div class="col-6">
                            ...
                            <select name="brand_product_id" id="brand_product_id" class="miSelect2"
                                style='width: 250px; margin-left:5px' name="state">
                                <option disabled selected value="0" id='default2' style="display:none"> Elija
                                    marca... </option>
        ...
        <tbody>
            @foreach ($products as $product)
                <tr id="{{ 'row' . $product['id'] }}">
                    ...
                    <td>{{ date('d-M-Y', strtotime($product['date_purchase'])) }}</td>
                    {{-- <td>{{ $product['status'] }}</td> --}}
                    <td>{{ $product['category']['name'] ?? ''}}</td>
                    <td>{{ $product['brand_product']['name'] ?? ''}}</td>

                    <td>
                        <button id='btn_update' value="{{ $product['id'] }}" class='btn' type="button"
                            data-toggle="modal" data-target="#updateModal">
                            <i class='fa fa-pen text-info'> </i>
                        </button>
                        <button id='btn_delete' value="{{ $product['id'] }}" class='btn' type="button"
                            data-toggle="modal" data-target="#updateModal">
